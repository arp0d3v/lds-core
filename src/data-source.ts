...
export class ListDataSource<T> {
    ...
    setFields(fields: LdsField[], callStateChanged?: boolean): void {
        ...
        this.fieldListBackup = fields.map(f => {
            const clone = new LdsField(f.name, f.title, f.dataType, f.visible, f.orderable, f.order1Name, f.order1Dir, f.order2Name, f.order2Dir);
        ...
        for (let i = 0; i < fields.length; i++) {
            ...
            if ((fieldData instanceof LdsField) === false) {
                field = new LdsField(fieldData.name, fieldData.title, fieldData.dataType, fieldData.visible, fieldData.orderable, fieldData.order1Name);
            ...
            if (field.orderable !== false) {
                if (field.order1Name === undefined) { field.order1Name = field.name; }
                if (field.order1Dir === undefined) { field.order1Dir = this.config.sort.defaultDir; }
                if (field.order2Dir === undefined) { field.order2Dir = this.config.sort.defaultDir; }
                this.sortList1.push(field);
                this.sortList2.push(field);
                if (field.name == this.state.order1Name) { this.sortItem1 = field; }
                if (field.name == this.state.order2Name) { this.sortItem2 = field; }
            }
    ...
    reload(eventName?: any): void {
        ...
        if (this.state.order1Dir === undefined) { this.state.order1Dir = this.config.sort.defaultDir; }
        if (this.state.order1Name === undefined) { this.state.order1Name = this.config.sort.defaultName; }
        if (this.hasData) {
            this.updatePaginationState();
        }
    ...
    changeSort(fieldName?: string | undefined, direction?: string | undefined): void {
        ...
        if (state.order1Name == fieldName) {
            state.order1Dir = this._toggleSortDir(state.order1Dir);
        } else {
            state.order1Name = fieldName;
            state.order1Dir = direction || this.config.sort.defaultDir;
        }
    ...
    private _toggleSortDir(dir?: string | null): string {
    ...
    clearState() {
        this.state.order1Dir = null;
        this.state.order2Dir = null;
        this.state.order1Name = null;
        this.state.order2Name = null;
        this.state.pagination.pageIndex = 0;
